...
const BlockProperties = () => {
  const selectedId = useProjectStore(s => (s as any).selectedBlockId) as string | null;
  const blocks = useProjectStore(s => (s as any).blocks) as any[];
  const setBlockPosition = useProjectStore(s => (s as any).setBlockPosition) as (id: string, p: { x: number; y: number; z: number }) => void;
  const setBlockColor = useProjectStore(s => (s as any).setBlockColor) as (id: string, color: number) => void;
  ...
  const setBlockRotationY = useProjectStore(s => (s as any).setBlockRotationY) as (id: string, rotationY: number) => void;
  const setBlockRotationX = useProjectStore(s => (s as any).setBlockRotationX) as (id: string, rotationX: number) => void;
  const setBlockRotationZ = useProjectStore(s => (s as any).setBlockRotationZ) as (id: string, rotationZ: number) => void;
  const setBlockScale = useProjectStore(s => (s as any).setBlockScale) as (id: string, scale: number) => void;
          ...
          <div className="space-y-2 pt-2 border-t border-sidebar-border">
            ...
            <div className="flex items-center gap-2 text-xs">
              ...
              <Button size="sm" variant="outline" onClick={() => {
                try {
                  const blocks = (useProjectStore.getState().blocks || []) as any[];
                  const next = blocks.map(b => b.id === block.id ? { ...b, mechanic, mechanicPower: mechanicPower } : b);
                  useProjectStore.getState().setBlocks(next as any);
                } catch {}
              }}>Apply</Button>
            </div>
          </div>
